...
@section('content')
                    ...
                    <ol itemscope="" itemtype="http://schema.org/BreadcrumbList">
                        ...
                        <li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
                            <a itemprop="item" href="">
                                <span itemprop="name">{{get_trans($product)}}</span>
                            </a>
                                        ...
                                        <div class="images-container list_thumb">
                                            <div class="product-cover">
                                                <img class="js-qv-product-cover img-fluid"
                                                     src="{{get_images($product ->images[0]->image_name) ?? ''}}"
                                                        ...
                                                        @foreach($product ->images as $image)
                                                            <div class="item thumb-container col-md-6 col-xs-12 pt-30">
                                                                <img class="img-fluid thumb js-thumb  selected "
                                                                     data-image-medium-src="{{get_images($image->image_name)}}"
                                                                     data-image-large-src="{{get_images($image->image_name)}}"
                                                                     src="{{get_images($image->image_name)}}"
                                            ...
                                            <form action="#"
                                                ...
                                                <div class="productdetail-right col-12 col-lg-6 col-md-6">
                                                    ...
                                                    <h1 class="detail-product-name"
                                                        itemprop="name">{{get_trans($product)}}</h1>
                                                    <div
                                                        ...
                                                        <div class="product-prices">
                                                            <div class="product-price " itemprop="offers" itemscope=""
                                                                ...
                                                                <div class="current-price">
                                                                    <span itemprop="price"
                                                                          class="price">{{number_format($product ->special_price) ?? number_format($product->price) }}</span>
                                                                    @if($product ->special_price)
                                                                        <span
                                                                            class="regular-price">{{number_format($product ->price)}}</span>
                                                                    @endif
                                                                </div>
                                                            </div>
                                                            <div class="tax-shipping-delivery-label">
                                                                {{ trans('site/home.Tax included') }}
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="in_border end">
                                                        ...
                                                        <div class="sku">
                                                            <label class="control-label">{{ trans('site/home.product code') }}:</label>
                                                            <span itemprop="sku"
                                                                  content="demo_1">{{$product ->sku ?? '--'}}</span>
                                                        </div>
                                                        <div class="pro-cate">
                                                            <label class="control-label">{{ trans('sections.section Name') }}:</label>

                                                            @isset($product ->sections)
                                                                <div>
                                                                    @foreach($product ->sections as $section )
                                                                        <span><a
                                                                                href="#"
                                                                                title="Computer &amp; Networking">{{$section ->name}}</a></span>
                                                                    @endforeach
                                                                </div>
                                                            @endisset
                                                        </div>
                                                        <div class="pro-tag">
                                                            <label class="control-label">{{ trans('tags.tags') }}:</label>
                                                            @isset($product ->tags)
                                                                <div>
                                                                    @foreach($product ->tags as $tag )
                                                                        <span><a href="">{{get_trans($tag)}}</a></span>
                                                                    @endforeach
                                                                </div>
                                                            @endisset
                                                        </div>
                                                    ...
                                                    <div id="_desktop_productcart_detail">
                                                        <div class="product-add-to-cart in_border">
                                                            <div class="add" style="{{ $product->in_stock !=1 ?'display:none':''  }}"  >
                                                                <button class="btn btn-primary add-to-cart cart-addition "
                                                                data-product-id="{{$product->id}}"
                                                                    ...
                                                                    <span>{{ trans('site/home.Add to cart') }}</span>
                                                                </button>
                                                            </div>

                                                            <a class="addToWishlist  wishlistProd_22" href="#"
                                                            style="{{ $product->in_stock !=1 ?'display:none':''  }}"
                                                               data-product-id="{{$product->id}}"
                                                            >
                                                            ...
                                                            <div id="product-availability" class="info-stock mt-20">
                                                                <label class="control-label">{{ trans('site/home.Availability') }}:</label>
                        ...
                        <div class="container">
                            ...
                            <section class="relate-product product-accessories clearfix">
                                ...
                                <div class="products product_list grid owl-carousel owl-theme" data-autoplay="true"
                                     data-autoplaytimeout="6000"  data-items="5" data-margin="0"
                                    ...
                                    <div class="item  text-center">
                                        <div class="product-miniature js-product-miniature item-two first_item"
                                             data-id-product="{{$product ->id }}" data-id-product-attribute="60" itemscope=""
                                             itemtype="http://schema.org/Product">
                                            <div class="product-cat-content">
                                                <div class="category-title">
                                                <div class="product-title" itemprop="name"><a
                                                        href="#">{{ get_trans($product) }}</a></div>
                                            ...
                                            <div class="thumbnail-container">
                                                ...
                                                <a href="{{ route('product_details',$product->slug) }}"
                                                   class="thumbnail product-thumbnail two-image">
                                                    ...
                                                    @endisset
                                                         alt=""
                                                         data-full-size-image-url="{{get_images($product -> images[0] ->image_name) ?? ''}}"
                                                         width="600" height="600">
                                                    <img class="img-fluid image-secondary"
                                                    @isset($product ->images[1])
                                                    src="{{get_images($product ->images[1] ->image_name) ?? ''}}"
    ...
    @include('site.products.product_details')
    @include('site.products.not-loggedin-modal')
    @include('site.products.alert-modal')   <!-- we can use only one with dynamic text -->
